...
def emit_turn(langfuse: Langfuse, session_id: str, turn_num: int, turn: Turn, transcript_path: Path,
              trace_prefix: str = "Claude Code", extra_tags: Optional[List[str]] = None) -> None:
    ...
    with propagate_attributes(
        ...
        tags=["claude-code"] + (extra_tags or []),
    ):
        trace_span = _start_backdated(
            langfuse,
            name=f"{trace_prefix} - Turn {turn_num}",
            as_type="span",
            start_time=user_ts,
...
def process_transcript(langfuse: Langfuse, session_id: str, transcript_path: Path,
                       state: Dict[str, Any], trace_prefix: str = "Claude Code",
                       extra_tags: Optional[List[str]] = None) -> int:
    """Read new rows from one transcript file and emit its turns. Returns turns emitted.

    Mutates `state` (per-file offset/turn_count); caller is responsible for save_state().
    """
    ...
    for t in turns:
        ...
        try:
            emit_turn(langfuse, session_id, turn_num, t, transcript_path,
                      trace_prefix=trace_prefix, extra_tags=extra_tags)
        except Exception as e:
...
def find_subagent_files(transcript_path: Path) -> List[Path]:
    """Subagent transcripts live in a directory named after the session, next to its .jsonl."""
    d = transcript_path.parent / transcript_path.stem / "subagents"
    if not d.is_dir():
        return []
...
def main() -> int:
    ...
    langfuse = None
    try:
        langfuse = Langfuse(public_key=public_key, secret_key=secret_key, host=host, environment=os.environ.get("LANGFUSE_TRACING_ENVIRONMENT", "mac"))
    except Exception:
        return 0
    ...
    try:
        with FileLock(LOCK_FILE):
            ...
            emitted = process_transcript(langfuse, session_id, transcript_path, state)

            # Subagent/workflow transcripts (Agent tool, ultracode workflows) carry
            # their own token usage that never appears in the main transcript.
            sub_emitted = 0
            for af in find_subagent_files(transcript_path):
                label = af.stem.removeprefix("agent-")
                sub_emitted += process_transcript(
                    langfuse, session_id, af, state,
                    trace_prefix=f"Subagent {label}",
                    extra_tags=["subagent"],
                )
        ...
        dur = time.time() - start
        info(f"Processed {emitted} turns + {sub_emitted} subagent turns in {dur:.2f}s (session={session_id})")
        return 0
