...
| –    | `GET /v1/auth/session`, `POST /v1/auth/logout` | Bearer-token session (stored hashed; 30 days idle, 90 days max).                                                                                                             |
...
- **Links.** The app generates a 256-bit link key and a 128-bit share id, encrypts the item, and builds `https://<share page>/#<id>.<key>`. The whole link lives in the fragment, which browsers never send anywhere. From the key it derives an encryption key and an access token (HKDF-SHA256); the API stores the ciphertext and `SHA-256(access token)` only. Opening a link (`POST /v1/shares/links/:id/open`) requires the token, counts one view, and deletes the ciphertext when the view limit is reached or the link is revoked. Expiry is 5 minutes to 30 days, views 1 to 100. The recipient page waits for a click before opening, so mail scanners and link previews do not use up views. Links are stored in Postgres, so they survive restarts and deploys.
- **Links for specific people.** The sender can limit a link to a list of emails (up to 20). The recipient opens the link, enters their email and gets a 6-digit code by email (10 minutes, single use, 5 wrong tries, a new one at most every 30 seconds and 5 an hour). The API releases the ciphertext only for the link's access token plus a live code for a listed email, and each view needs a new code. Emails and codes are stored only as HMACs keyed with `SERVER_SECRET` and salted with the link id; the code request answers the same for any email. The key still lives only in the fragment, so the app hands the link to the sender's own mail app (`mailto:`) rather than having Zvault send it. With SES in sandbox mode, codes only reach verified addresses.
